...
import (
	...
	rpCore "parking/api/core/report"
	pvCore "parking/api/core/vehicle"
	plSvc "parking/api/services/parkinglot"
	psSvc "parking/api/services/parkingslot"
	rpSvc "parking/api/services/report"
...
func setupGRPCServices(config *viper.Viper, logger *logrus.Entry, store *postgres.Storage) *Svcs {
	...
	pvSvc := pvSvc.New(pvCore.New(store, logger))
	rpSvc := rpSvc.New(rpCore.New(store, logger))
	return &Svcs{
		external: []Service{plSvc, psSvc, pvSvc, rpSvc},
		internal: []Service{plSvc, psSvc, pvSvc, rpSvc},
	}
}
